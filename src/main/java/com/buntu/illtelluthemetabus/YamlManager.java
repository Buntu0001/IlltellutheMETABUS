...
public class YamlManager {
  ...
  private void yamlInitialize() {
    if (!questionFile.exists()) {
      ...
      questionYmlFile.set("question.기본문제.문제해설", lores.toArray());
      ...
      lores.add("&f이것은 해설일까 아닐까?");
      lores.add("&f그것은 아무도 모른다.");
      questionYmlFile.set("question.심화문제.문제해설", lores.toArray());
  ...
  public void yamlRetrieveData() {
    try {
      ...
      for (String node : questionYmlFile.getConfigurationSection("question").getKeys(false)) {
        ...
        ArrayList<String> questionContextLores = new ArrayList<>(
            questionYmlFile.getStringList("question." + node + ".문제상세내용"));

        ArrayList<String> questionCommentary = new ArrayList<>(
            questionYmlFile.getStringList("question." + node + ".문제해설"));
        ...
        Question question = new Question(questionTitle, questionContext, questionContextLores, questionCommentary,
            questionOptions,
            difficulty, questionAnswer, questionScore, questionLimitTime);
